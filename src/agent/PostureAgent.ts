...
export class PostureAgent {
  private ollamaClient: OllamaClient;
  private dbClient: DatabaseClient;
  private sopCache: SOPRule[] = [];
  ...
  private async warmUpCache(): Promise<void> {
    const dummyPosture: AveragedPosture = { neck_angle: 90, back_angle: 90, head_forward_distance: 5 };
  ...
  async generate(posture: AveragedPosture): Promise<PostureAdvice> {
    try {
  ...
  private async callGenerator(posture: AveragedPosture): Promise<GeneratorOutput> {
  ...
  private async callReflector(posture: AveragedPosture, generatorOutput: GeneratorOutput): Promise<ReflectorOutput> {
...
Provide improved final version.`;
  ...
  private buildUserPrompt(posture: AveragedPosture): string {
    return `POSTURE DATA:
${this.formatPostureForPrompt(posture)}
...
Generate a 3-step corrective action plan.`;
  ...
  private formatPostureForPrompt(posture: AveragedPosture): string {
    return `Neck Angle: ${posture.neck_angle}°
Back Angle: ${posture.back_angle}°
Head Forward Distance: ${posture.head_forward_distance}cm`;
  }

  private formatSOPRules(rules: SOPRule[]): string {
    if (rules.length === 0) return 'No SOP rules available.';
  ...
  private parseGeneratorResponse(content: string): GeneratorOutput {
    try {
  ...
  private parseReflectorResponse(content: string): ReflectorOutput {
    try {
      const parsed = JSON.parse(content);
      return {
        steps: Array.isArray(parsed.steps) ? parsed.steps.slice(0, 3) : [],
  ...
  private getFallbackAdvice(): PostureAdvice {
    const fallback = OllamaClient.getFallbackAdvice();
    return {
  ...
  private averageFrames(frames: { neck_angle: number; back_angle: number; head_forward_distance: number }[]): AveragedPosture {
    if (frames.length === 0) {
