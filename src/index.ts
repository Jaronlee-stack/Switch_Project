import { PostureAgent } from './agent/PostureAgent.js';
import { OllamaClient } from './llm/ollamaClient.js';
import { DatabaseClient } from './db/client.js';
import { AgentConfig, DatabaseConfig } from './types/index.js';

function loadConfig(): { agent: AgentConfig; db: DatabaseConfig; pollInterval: number } {
  return {
    agent: {
...
async function main(): Promise<void> {
  ...
  const ollamaClient = new OllamaClient(config.agent);
  const agent = new PostureAgent(ollamaClient, dbClient, config.agent);
